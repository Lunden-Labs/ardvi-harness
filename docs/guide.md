...
To replace a temporary patched installation, install the v0.4.1 host CLI, then
run `make update` in each project. Updating only
the MCP service does not replace the host executable. Existing
